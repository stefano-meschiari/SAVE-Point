...
if ($_GET['action'] == 'load') {
  $cur = pg_query_params("SELECT mission_data FROM orbits_data WHERE user_id=$1", array($user_id));
  ...
  if (pg_num_rows($cur) === 0) {
    $cur = pg_query_params("INSERT INTO orbits_data (user_id, mission_data, earned_stars) VALUES ($1, $2, $3)",
    ...
    $cur = pg_query_params("UPDATE orbits_data SET (mission_data, earned_stars) = ($1, $2) WHERE user_id=$3",
  ...
  pg_query_params("DELETE FROM orbits_data WHERE user_id=$1", array($user_id));
  die();
}
