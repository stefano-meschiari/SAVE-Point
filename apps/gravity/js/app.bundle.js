...
var $ = require('jquery'),
    ...
    Physics = require('../../share/js/physics'),
    _m = require('../../share/js/math'),
    draw = require('./draw');
...
var MissionHelpView = Backbone.View.extend({
    ...
    templater: {
        ...
        "@nextmission": '<button class="btn btn-lg next-mission pull-right">Next mission</button>',
        "@wait-1": '<script> _.delay(function() { missionHelpView.next();  }, 1000); </script>',
        "@wait-5": '<script> _.delay(function() { missionHelpView.next();  }, 5000); </script>',
        "@wait-30": '<script> _.delay(function() { missionHelpView.next();  }, 30000); </script>'
    },
    ...
    initialize: function() {
        ...
        this.listenToOnce(this.model, "change:currentMission change:missions", this.setupTemplates);
        this.listenTo(this.model, "change:currentMission change:missions", this.render);

        var self = this;
        $("#help-next").on("click", function() { self.next(); });
    },
        ...
        for (var i = 0; i < missions.length; i++) {
            ...
            for (var j = 0; j < m.help.length; j++) {
                ...
                m.help[j].message = _.reduce( _.keys(templater), function(transformed, tag) {
                    return transformed.replace(new RegExp(tag, 'g'), templater[tag]);
                }, m.help[j].message);
           };
        }
    },
    ...
    render: function() {
        var helpNum = this.model.get('missions')[this.model.get('currentMission')].help.length;

        if (this.currentHelp < helpNum-2) {
            $("#help-nav").show();
            $("#help-next").html(this.NEXT_BUTTON_LABEL);
        }
        else if (this.currentHelp == helpNum-2) {
            $("#help-nav").hide();
        } else {
            $("#help-nav").show();
            $("#help-next").html(this.NEXT_MISSION_BUTTON_LABEL);
        }
        
        var mission = this.model.get('missions')[this.model.get('currentMission')];
        var helpText = mission.help[this.currentHelp].message;
        this.$el.html(helpText);
    },
    ...
    next: function() {
        ...
        $("#help-text").toggleClass("expanded");
...
},{"../../share/js/init":3,"../../share/js/math":4,"../../share/js/physics":5,"./draw":2,"backbone":7,"jquery":8,"underscore":10}],2:[function(require,module,exports){
var $ = require('jquery'),
    _ = require('underscore'),
    Backbone = require('backbone');
...
PIXELS_PER_AUPDAY = 100 / Math.sqrt(Units.K2);
// Size of arrow
ARROW_SIZE = 10;
...
//        textItem.opacity = 0;
    ...
    initialize: function() {
        ...
        this.listenTo(this.model, "change:currentMission change:missions", function() {
            var missionTitle = app.get('missions')[app.get('currentMission')].title;
            self.displayMessage("Hello!");
        });
...
$(document).ready(function() {
    ...
    view.onMouseDown = _.bind(draw.onMouseDown, draw);
});

},{"backbone":7,"jquery":8,"underscore":10}],3:[function(require,module,exports){
/*
 * Common initialization for all apps.
 */
...
},{"underscore":10,"underscore.string":9}],4:[function(require,module,exports){
"use strict";
...
},{"./math":4,"./units":6}],6:[function(require,module,exports){
"use strict";
